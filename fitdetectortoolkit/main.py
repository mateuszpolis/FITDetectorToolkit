...
class ModuleManager:
    ...
    def install_module(self, module_name: str) -> bool:
        ...
        try:
            ...
            Repo.clone_from(module_info["url"], module_path, branch=module_info["branch"])
            ...
            if not pyproject_toml.exists() and not setup_py.exists():
                raise FileNotFoundError(f"Neither pyproject.toml nor setup.py found in {module_name}")
            ...
            if pyproject_toml.exists():
                ...
                subprocess.run([sys.executable, "-m", "pip", "install", "-e", str(module_path)], 
                             check=True, capture_output=True)
            else:
                print(f"Warning: {module_name} uses setup.py (pyproject.toml preferred)")
    ...
    def is_module_installed(self, module_name: str) -> bool:
        ...
        return self.modules.get(module_name, {}).get("installed", False)
                    ...
                    try:
                        ...
                        if hasattr(module, 'main'):
                            ...
                            subprocess.Popen([sys.executable, "-c", 
                                           f"import {module_info['entry_point']}; {module_info['entry_point']}.main()"])
                            return True, "Module launched successfully"
                        else:
...
class FITDetectorToolkit:
    ...
    def configure_styles(self):
        ...
        self.style.configure('Module.TFrame', 
                           background='#3c3c3c', 
                           relief='raised', 
                           borderwidth=2)
        
        self.style.configure('Module.TLabel', 
                           background='#3c3c3c', 
                           foreground='#ffffff', 
                           font=('Arial', 10))
    ...
    def create_module_widgets(self, parent):
        ...
        for module_name, module_info in self.module_manager.modules.items():
            ...
            # Status indicator
            status_text = "✓ Installed" if module_info.get('installed', False) else "✗ Not Installed"
            status_color = '#4CAF50' if module_info.get('installed', False) else '#f44336'
            ...
            # Launch button
            launch_btn = ttk.Button(buttons_frame, 
                                  text="Launch", 
                                  style='Module.TButton',
        ...
        def install_thread():
            try:
                ...
                if success:
                    ...
                    try:
                        messagebox.showinfo("Success", f"{module_name} has been installed successfully!")
                    except tk.TclError:
        ...
        def launch_thread():
            try:
                ...
                if success:
                    ...
                    try:
                        messagebox.showerror("Error", f"Failed to launch {module_name}: {message}")
                    except tk.TclError:
                        # Window was closed, just update status
                        pass
            except Exception as e:
...
Examples:
  ...
  fitdetectortoolkit --install AgeingAnalysis  # Install a module
    ...
    parser.add_argument(
        '--install', 
        metavar='MODULE',
        help='Install a specific module'
    )
    
    parser.add_argument(
        '--launch', 
        metavar='MODULE',
        help='Launch a specific module'
    )
    
    parser.add_argument(
        '--version', 
        action='version', 
        version='FITDetectorToolkit 0.1.0'
    )
